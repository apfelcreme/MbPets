...
public class CancelPreparationCommand implements SubCommand {
    ...
    public void execute(ChatInput chatInput) {
        ...
        if (MbPets.getInstance().getConfigurations().containsKey(chatInput.getSender())) {
            MbPets.getInstance().getConfigurations().remove(chatInput.getSender());
            chatInput.getSender().sendMessage(MbPetsConfig.getTextNode("info.preparedPetCanceled"));
        } else {
            chatInput.getSender().sendMessage(MbPetsConfig.getTextNode("error.noPreparedPets"));
        }
    }
}
