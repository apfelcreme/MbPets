...
public class WolfPet extends Pet implements Ageable, Dyeable<AnimalColor> {

	AnimalColor color = null;
	Boolean isBaby = null;	

	public WolfPet(OfflinePlayer owner, String name, Integer number, boolean isBaby, AnimalColor color) {
		super(owner, name, DisguiseType.WOLF, number);
		this.isBaby = isBaby;
		this.color = color;
	}	
	...
	public FlagWatcher prepareWatcher(FlagWatcher watcher) {
		((WolfWatcher)watcher).setBaby(isBaby);
		((WolfWatcher)watcher).setCollarColor(color);
		((WolfWatcher)watcher).setCustomName(name);
		return watcher;
	}

	@Override
	public PreparedStatement prepareConfirmStatement(Connection connection) throws SQLException {
		PreparedStatement statement = connection.prepareStatement("INSERT INTO MbPets_Pet(playerid, petname, type, wolfcolor, baby, number)"
				+ " VALUES ("
